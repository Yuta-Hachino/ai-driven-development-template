...
4. Execute GoReleaser
   ...
   │  ├─ Upload checksums
   │  └─ Generate changelog
   ├─ Update Homebrew tap (autonomous-dev/homebrew-tap)
   │  └─ Commit updated Formula/autonomous-dev.rb
...
# Check version
brew info autonomous-dev/tap/autonomous-dev
# Should show: autonomous-dev 1.0.1

# Install/upgrade
brew install autonomous-dev/tap/autonomous-dev
# or
brew upgrade autonomous-dev/tap/autonomous-dev
...
| `HOMEBREW_TAP_TOKEN` | ⚠️ Optional | Update Homebrew tap | [GitHub Personal Access Token](https://github.com/settings/tokens) with `repo` scope |
...
#### Creating HOMEBREW_TAP_TOKEN (Optional)

If you want automatic Homebrew tap updates:
...
4. Generate token
5. Add to GitHub Secrets as `HOMEBREW_TAP_TOKEN`

**Note**: If `HOMEBREW_TAP_TOKEN` is not set, GoReleaser will use `GITHUB_TOKEN` (may have permission issues for separate tap repository).
...
```bash
# Create repository: https://github.com/autonomous-dev/homebrew-tap
# (Must be public)

# Repository structure:
homebrew-tap/
└── Formula/
    └── autonomous-dev.rb   # Auto-generated by GoReleaser
```

GoReleaser will automatically create/update `Formula/autonomous-dev.rb` on each release.
...
# 4. Revert Homebrew tap (if needed)
# Manually edit autonomous-dev/homebrew-tap Formula/autonomous-dev.rb
...
**Solution**:
1. Verify `HOMEBREW_TAP_TOKEN` has `repo` scope
2. Check `autonomous-dev/homebrew-tap` repository exists and is public
3. Ensure token owner has write access to tap repository
...
3. Users update: `brew upgrade` or `npm update`

✅ **Zero Manual Steps** (after initial setup)

✅ **Multi-Platform Support**: macOS, Linux, Windows (6 architectures)

✅ **Multi-Channel Distribution**: GitHub, Homebrew, npm, apt/yum
